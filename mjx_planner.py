...
class cem_planner():
	...
	def __init__(self, num_dof=None, num_batch=None, num_steps=None, timestep=None, maxiter_cem=None, num_elite=None, w_pos=None, w_rot=None, w_col=None, 
		...
		self.model_path = f"{os.path.dirname(__file__)}/default.xml" 
	...
	def cem_iter(self, carry,  scan_over):
		...
		#xi_filtered = xi_filtered.reshape(self.num_batch, self.num_steps* self.num_dof)
	...
	def compute_cem(
		...
		best_traj = base_pos[-1][idx_min].reshape((self.num_steps, 3))  # Assuming x,y,theta trajectory
...
def main():
	...
	opt_class = cem_planner(num_dof=num_dof, num_batch=num_batch, num_steps=50, maxiter_cem=1,
                           w_pos=1, w_rot=0.5, w_col=10, num_elite=0.05, timestep=0.05,
						   maxiter_projection=5, max_pos = np.pi, max_vel=2.0, max_acc=5.0, max_jerk=10.0)
	...
	init_vel = jnp.array([0.0, 0.0]) #linear velocity and angular velocity
	init_acc = jnp.array([0.0])
	target_pos = jnp.array([0.5, 0.5, 0.0])
